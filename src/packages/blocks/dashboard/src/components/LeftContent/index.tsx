...
const LeftContent: React.FunctionComponent<LeftContentProps> = ({
  data,
  onSelectSubCategory,
  banners,
  onSelectCategory,
  ...
  return (
    <aside className="h-100 d-flex flex-column">
                  ...
                  <Accordion
                    ...
                    onHeaderClick={() => {
                      setActiveSubCategory(
                        activeSubCategory === category.attributes.id
                          ? null
                    ...
                    content={
                      <SimpleVerticalNav
                        list={category.attributes.sub_categories.map(
                          (subcat: any) => ({
                            label: subcat.name,
                            value: subcat.id,
                            navItem: subcat,
      ...
      <div className="home-promotion-banner">
        {banners?.length > 0 && (
          <PromotionBanner src={banners[0].attributes?.url} label="Promotion" />
        )}
      </div>
    </aside>
  );
