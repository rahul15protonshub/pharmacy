...
import { Alert, Col, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, Row, Container } from "reactstrap";
...
const RightContent: React.FunctionComponent<RightContentProps> = (props: RightContentProps) => {
    const {
        ...
        <div className='right-content'>
            ...
            <div className='product_catergory'>
                <div className="yt-sp-mb-filter-wrapper align-self-center">
                    <button className="d-flex align-items-center product_filter" onClick={() => toggleOurProducts()}>
                        <div className="yt-sp-recmnd-icn">
                            <BsFunnel />
                        </div>
                        <div className="yt-recmnd-mb-txt filter_type"> Our products</div>
                    </button>
                    <div className='product_category_type_web'>
                        {productListTitle}
                    </div>
                </div>

                <div style={{ margin: "6px", display: "flex", alignItems: "center" }}
                    ...
                    >
                        <DropdownToggle
                            caret className='yt-product-page-recommend-btn'
                            style={{ display: "flex", minWidth: "12 rem" }}>
                            <span className="sort_type">
                        ...
                        <DropdownMenu className="yt-recommend-wrapper">
                            {sortMenu.map(e => (
                                <DropdownItem
                                    key={e.label}
                                    className={e.label === selectedFilter.label ? "current-active-filter active" : ""}
                                    onClick={() => {
                                        setSelectedFilter(e)
            ...
            <div className='product_category_type'>
                {productListTitle}
            </div>
            <div className='home-main-product-list'>
                ...
                <Container fluid className='p-0'>
                            ...
                            products?.length === 0 ? (
                                ...
                                products?.map(product => (
                                    <Col lg="3" md="4" xs="6" key={product.id} className="d-flex">
                                        <ProductBox onProductAddToWishlist={onProductAddToWishlist}
                    ...
                    <Row>
                        {loading && <Col className='flex-center'><CircularProgress /></Col>}
                    </Row>
                </Container>
            </div>
        </div>
