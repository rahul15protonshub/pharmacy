    ...
    >
      <div className="yt-mobile-side-nav-wrap">
        <div className="yt-mobile-side-inner-content">
          <div className="yt-inner-cnt-logo">
            <div className="logobox-mb">
              ...
              />
              <h3>{JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.ExtraFields?.heading ? JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.ExtraFields?.heading : "Builder Fan Club"}</h3>  
            </div>
          </div>
        ...
        if (!isEmpty(localStorage.getItem("token")) && !isEmpty(localStorage.getItem("guestUUID")) && isEmpty(localStorage.getItem("userData"))) {
          props.history.push({ pathname: "/", state: { "calledFrom": "cart", "redirect": "wishlist" } });
          toggle();
        }
        else {
          //@ts-ignore
                      ...
                      onClick={() => {
                        ...
                        if (localStorage.getItem('token')) {
                            ...
                            props.history.location.pathname.split("/").join(",").length < 1
                              ?
                              props.history.push(`./Filteroptions?&page=${1}&per_page=${15}&sort[order_by]=sold&sort[direction]=desc`)
                              :
                              props.history.push(`./${route.repeat(props.history.location.pathname.split("/").join(",").length - 1)}Filteroptions?&page=${1}&per_page=${15}&sort[order_by]=sold&sort[direction]=desc`)
                          toggle();
                        }
                        else {
                          //@ts-ignore 
          ...
          <div style={{width:'93%',margin:'0 auto'}}>
            <Row className="align-items-center">
              ...
              <Link to={localStorage.getItem('token') ? '/home-page' : '/'}>
                <div className="d-flex align-items-center"  >
                    <div className="logobox">
                      ...
                      <img
                        src={
                          JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.commonLogoSrc ? JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.commonLogoSrc : require('./images/logo.svg')}
                        className="logoimage"
                        alt="studio store"
                      />
              ...
              <Col  xs={1} md={3} lg={1} className="yt-head-col logo-txt">
                <Link to={localStorage.getItem('token') ? '/home-page' : '/'}>
                    <div >
                      <span>{JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.ExtraFields?.heading ? JSON.parse(localStorage.getItem("appThemData") ?? "{}")?.ExtraFields?.heading : "Builder Fan Club"}</span>
                    </div>
                  </Link>
              </Col>
                      ...
                      onClick={() => {
                        ...
                        if (localStorage.getItem('token')) {
                          ...
                          this.props.history.location.pathname.split("/").join(",").length < 1
                            ?
                            this.props.history.push(`./Filteroptions?&page=${1}&per_page=${15}&sort[order_by]=sold&sort[direction]=desc`)
                            :
                            this.props.history.push(`./${route.repeat(this.props.history.location.pathname.split("/").join(",").length - 1)}Filteroptions?&page=${1}&per_page=${15}&sort[order_by]=sold&sort[direction]=desc`)
                        } else {
                          //@ts-ignore 
                          window.notify([{ type: 'danger', message: 'Please Login' }]);
                        }
...
<div className="cartcontainer py-2" style={{
  ...
  <div className="d-flex">
    ...
    <span className="d-inline-block yt-cart-icn-mainwp">
      <svg className="favoriteicon w3-ripple" onClick={() => {
          if (!isEmpty(localStorage.getItem("token")) && !isEmpty(localStorage.getItem("guestUUID")) && isEmpty(localStorage.getItem("userData"))) {
        ...
        }}   width="21" height="21" viewBox="0 0 24 24" fill="#fff" stroke="currentcolor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
      {/* <FaRegHeart
      ...
      /> */}
      {parseInt(this.state.wishlistLength) > 0 && <span className="w3-green cart-notify wishlist">
        {/* @ts-ignore */}
    ...
    <span className="d-inline-block yt-cart-icn-mainwp">
      <svg width="24" height="24" onClick={() => {
        ...
        }} className="carticon w3-ripple" viewBox="0 0 24 24" fill="#fff" xmlns="http://www.w3.org/2000/svg">
