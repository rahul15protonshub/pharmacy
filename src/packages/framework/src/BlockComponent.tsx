  ...
  parseExpireTokenResponse(responseJson:any,state:any,props:any){
    if (responseJson?.errors) {
      if (!state.invalidTokenMessageRecieved) {
          if (Array.isArray(responseJson?.errors) && (responseJson?.errors.length > 0) && responseJson?.errors[0]?.token && (responseJson?.errors[0]?.token == "Token has Expired" || responseJson?.errors[0]?.token == "Invalid token")) {
              //@ts-ignore
              localStorage.clear();
              props?.history?.push({ pathname: '/login', state: { activeTab: "2" } });
              return false
            }
            return true
      }
